...
namespace AE_TnN_Mobile_BLL
{
    public class clsCase
    {
        ...
        public DataTable ExecuteSQLQuery(string sQuery, ref string sErrDesc)
        {
        ...
        public DataTable Run_SQLQueryString_DataTable(string sQuery, string sConnString)
        {
        ...
        //    return sResult;
        //}
        public string SPA_TimeEntry_LawMini(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            ...
            try
            {
                sFuncName = "SPA_LawyerFee_LawMini()";
                if (p_iDebugMode == DEBUG_ON) oLog.WriteToDebugLogFile("Starting Function ", sFuncName);
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    string PostingDate = dr["Date"].ToString();
                   // string Curr = dr["Curr"].ToString();
                    string Task = dr["Task"].ToString();
                    int BillUnit = Convert.ToInt16(dr["BillUnit"]);
                    int Dur = Convert.ToInt16(dr["Dur"]);
                    string Remarks = dr["Remarks"].ToString();//Billable
                    string Billable = dr["Billable"].ToString();//Billable
                    //Double Amount = Convert.ToDouble(dr["Amount"]);
                    // oLogin.ConnectToTargetCompany("");
                    oDICompany = oLogin.ConnectToTargetCompany(ConnectionString);
                    if (oDICompany.Connected == false)
                    {
                        throw new System.ArgumentException("Company not connected");
                    }

                    SAPbobsCOM.Documents oSO = (SAPbobsCOM.Documents)(oDICompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oOrders));
                    oSO.CardCode = CardCode;
                    oSO.DocDate = DateTime.Now;
                    oSO.DocDueDate = DateTime.Now;
                    oSO.Lines.ItemCode = "200001";
                    oSO.Lines.ItemDescription = Task;
                    oSO.Lines.Quantity = BillUnit;
                    oSO.Lines.UserFields.Fields.Item("U_BILLABLE").Value = Billable;
                    oSO.Lines.UserFields.Fields.Item("U_DURATION").Value = Dur;
                    //oSO.Lines.UnitPrice = 15;
                    oSO.Lines.Add();
                    ...
                    //RetDt.TableName = "TIME";
                    //RetDT1 = RetDt.Copy();
                    //RetDS.Tables.Add(RetDT1);
                    //sResult = fn.ds2json(RetDS);
                    //}
        ...
        public string SPA_ExpenseEntry_LawMini(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                   ...
                   // oLogin.ConnectToTargetCompany("");
                    ...
                    oSO.DocDate = DateTime.Now;
                    oSO.DocDueDate = DateTime.Now;
                    oSO.Lines.ItemCode = Expense;
                    oSO.Lines.Quantity = 1;
                    oSO.Lines.UnitPrice = Amount;
                    oSO.Lines.Add();
                    oSO.Comments = Remarks;
                    RetCode = oSO.Add();
        ...
        public string SPA_Get_ExpenseEntry_LawMini(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                sFuncName = "SPA_Get_ExpenseItems_LawMini()";
                if (p_iDebugMode == DEBUG_ON) oLog.WriteToDebugLogFile("Starting Function ", sFuncName);
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    Str = "SELECT T0.[DocDate] [Date], T1.[Dscription] Expense, T1.[LineTotal] Expense, T1.[Currency] [Curr], T0.[Comments] [Remarks] FROM ORDR T0  INNER JOIN RDR1 T1 ON T0.[DocEntry] = T1.[DocEntry] WHERE T0.[CardCode] ='" + CardCode + "'";

                    RetDt = Run_SQLQueryString_DataTable(Str, ConnectionString);
                    RetDt.TableName = "EXPENSE";
                    RetDT1 = RetDt.Copy();
                    RetDS.Tables.Add(RetDT1);
                    sResult = fn.ds2json(RetDS);
        ...
        public string SPA_Get_TimeEntry_LawMini(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    Str = "SELECT T0.[DocDate] [Date], T1.[Dscription] [Task],T1.[Quantity] [BillUnit],  T1.[LineTotal] [BillableAmount], T1.[U_BILLABLE] [Billable], T1.[U_DURATION] [Dur], T0.[Comments] [Remarks] FROM ORDR T0  INNER JOIN RDR1 T1 ON T0.[DocEntry] = T1.[DocEntry] WHERE T0.[CardCode]  ='" + CardCode + "'";
        ...
        public string SPA_LawyerFee_LawMini(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    String CardCode = dr["MemeberID"].ToString();
                    CardCode = "1";

                    Str = "exec [getLawyerFee] '" + CardCode + "'";

                    RetDt = Run_SQLQueryString_DataTable(Str, ConnectionString);
                    //RetDS = Run_QueryString(Str, ConnectionString);
                    // sResult = fn.ds2json(RetDS);
                    //if (SErrMsg == "")
                    //{
                    RetDt.TableName = "TIME";
        ...
        public string SPA_Get_ExpenseItems_LawMini(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    Str = "exec [LITIGATION_getExpenseItems] '" + CardCode + "'";
        ...
        public string SPA_Get_TimeItems_LawMini(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    Str = "exec [LITIGATION_getTimesheetItems] '" + CardCode + "'";
                  ...
                  //  sResult = Regex.Unescape(sResult);
                  //  sResult = Regex.Replace(sResult, @"\t\n\r", "");
                   // sResult = Regex.Replace(sResult,  @"[\\r|\\n|\\t]", ""); 
                   //   sResult=sResult.Replace(@"\t|\n|\r", ""); 
                    //sResult = sResult.Replace(System.Environment.NewLine, string.Empty);
                    //sResult=sResult.Replace(""\"", string.Empty);
                    //sResult = sResult.TrimEnd('\r', '\n');
                    //sResult=sResult.Replace(@"\r\n      \",String.Empty);
                    //sResult = sResult.Replace("\n", String.Empty);
                    //sResult = sResult.Replace("\r", String.Empty);
                    //sResult = sResult.Replace("\t", String.Empty);
                    sResult = sResult.Replace(@"\", String.Empty);
                    sResult = sResult.Replace(@"/", String.Empty);
        ...
        public string SPA_Get_Email(DataTable dtDatatable, string sUserName, string FileUploadPath)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    Str = "SELECT T0.[ClgCode], Replace(convert(varchar, T0.[Recontact], 101),'/','-') [Recontact], T0.[Details] Email_subject, T0.[Notes] Email_Contents, T0.[Attachment],T2.[Name] Sender, T2.[E_MailL], T2.[Cellolar] FROM OCLG T0  INNER JOIN OCRD T1 ON T0.[CardCode] = T1.[CardCode] INNER JOIN OCPR T2 ON T0.[CntctCode] = T2.[CntctCode] where T0.[CardCode] = '" + CardCode + "' Order by ClgCode desc";
                    RetDt = Run_SQLQueryString_DataTable(Str, ConnectionString);
                    RetDt.TableName = "EMAIL";
                    RetDT1 = RetDt.Copy();
                    RetDS.Tables.Add(RetDT1);
                    //Out Look
                    DataTable dt = new DataTable("EMAILATT");
                    //Adding columns to table Emp
                    DataColumn ClgCode = new DataColumn("ClgCode", typeof(System.Int32));
                    DataColumn FilePath = new DataColumn("FilePath", typeof(System.String));
                    //Adding columns to datatable
                    dt.Columns.AddRange(new DataColumn[] { ClgCode, FilePath });

                    foreach (DataRow row in RetDt.Rows)
                    {
                        string ClgCode1 = row["ClgCode"].ToString();
                        string Attachment = row["Attachment"].ToString();
                        OutlookStorage.Message outlookMsg = new OutlookStorage.Message(@Attachment);
                        foreach (OutlookStorage.Attachment attach in outlookMsg.Attachments)
                        {
                            string fileName = @FileUploadPath + attach.Filename;
                            Directory.CreateDirectory(Path.GetDirectoryName(fileName));
                            byte[] attachBytes = attach.Data;
                            FileStream fs = new FileStream(fileName, FileMode.Create);
                            fs.Write(attachBytes, 0, attachBytes.Length);
                            fs.Close();
                            dt.Rows.Add(ClgCode1, fileName);
                        }
                    }

                    RetDT1 = new DataTable("EMAILATT");
                    RetDT1 = dt.Copy();
                    RetDS.Tables.Add(RetDT1);
                    sResult = fn.ds2json(RetDS);
                    return sResult;
        ...
        public string SPA_Get_blog(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    Str = "Select U_MESSAGE Message,U_MEMBERID MemeberCode,Replace(convert(varchar, U_DT_CREATE, 101),'/','-') CreateDate,U_CASENO CardCode,U_MEMBERNAME MemeberName from  [@AE_LAW_BLOG] where U_CASENO='" + CardCode + "'";
        ...
        public string SPA_Add_blog(DataTable dtDatatable, string sUserName)
        {
            //DataSet oDataset = new DataSet();
            //DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    String CardCode = dr["CardCode"].ToString();
                    String MemeberCode = dr["MemeberCode"].ToString();
                    String MemeberName = dr["MemeberName"].ToString();
                    String Message = dr["Message"].ToString();

                    Str = "insert into [@AE_LAW_BLOG](Code,Name,U_MESSAGE,U_MEMBERID,U_DT_CREATE,U_CASENO,U_MEMBERNAME) values ((select COUNT(Code) from [@AE_LAW_BLOG]),(select COUNT(Code) from [@AE_LAW_BLOG]),'" + Message + "','" + MemeberCode + "',Cast(CONVERT(date, getdate())as Varchar(15)),'" + CardCode + "','" + MemeberName + "')";
                    SqlConnection con = new SqlConnection(ConnectionString);
                    SqlCommand command = con.CreateCommand();
                    command.CommandText = Str;
                    if (p_iDebugMode == DEBUG_ON) oLog.WriteToDebugLogFile("Executing the Query : " + Str, sFuncName);

                    con.Open();
                    command.ExecuteNonQuery();
                    con.Close();
                    sResult = "Blog Added successfully for the Customer = " + CardCode;
                    return sResult;
                }
            }
            catch (Exception Ex)
        ...
        public string SPA_AddCase_LawMini(DataTable dtDatatable, string sUserName)
        {
            DataSet oDataset = new DataSet();
            DataTable dtAssignCase = new DataTable();
            string sFuncName = string.Empty;
            ...
            try
            {
                ...
                if (dtDatatable != null && dtDatatable.Rows.Count > 0)
                {
                    ...
                    String Str = string.Empty;
                    Str = "SELECT Count(T0.[CardCode]) TOTCOUNT FROM OCRD T0 WHERE T0.[CardType] ='C'";
                    SqlConnection con = new SqlConnection(ConnectionString);
                    SqlCommand command = con.CreateCommand();
                    command.CommandText = Str;
                    if (p_iDebugMode == DEBUG_ON) oLog.WriteToDebugLogFile("Executing the Query : " + Str, sFuncName);
                    ...
                    while (reader.Read())
                    {
                        CardCode = reader["TOTCOUNT"].ToString();

                    }
                    CardCode = "C0000" + CardCode;
                    con.Close();
                    if (p_iDebugMode == DEBUG_ON) oLog.WriteToDebugLogFile("Connecting to target company ", sFuncName);
                    oDICompany = oLogin.ConnectToTargetCompany(ConnectionString);
                    ...
                    if (oBP.GetByKey(CardCode.ToString()))
                    {
                        //exists
                        isExists = true;
                    }
                    ...
                    {
                        //not exists
                        oBP.CardCode = CardCode.ToString();
                    }

                    //  oBP.CardCode = dr["CardCode"].ToString();
                    oBP.CardName = dr["Matter"].ToString();
                    ...
                    //Plaintiff
                    if (dr["Plaintiff"].ToString() == "Y")
                    {
                        oBP.set_Properties(22, SAPbobsCOM.BoYesNoEnum.tYES);
                    }
                    if (dr["Defendant"].ToString() == "Y")
                    {
                        oBP.set_Properties(23, SAPbobsCOM.BoYesNoEnum.tYES);
                    ...
                    if (lRetCode == 0)
                    {
                        ...
                        //gopi
                        sResult = "Customer Added/ successfully for the CardName = " + dr["Matter"].ToString();
                    }
                    else
                    {
                        //       if (oDICompany.InTransaction) oDICompany.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_RollBack);
                    ...
                    if (item["IDType"].ToString() == "INDIVIDUAL")
                    {
                        ...
                        if (ExistingCheck.Rows.Count > 0)
                        {
                            if (ExistingCheck.Rows[0][0].ToString().Length == 0)
                            {
                                ...
                                oGeneralData.SetProperty("U_EMAIL", item["EMAIL"].ToString());
                                oGeneralService.Add(oGeneralData);
                            }
                            else
                            {
