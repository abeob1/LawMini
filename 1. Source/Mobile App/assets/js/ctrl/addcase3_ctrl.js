App.controller('addcase3_ctrl', ['$scope', '$rootScope', '$http', '$window', '$cookies','util_SERVICE','Addcase_SERVICE',
...
function ($scope, $rootScope, $http, $window, $cookies,US,ACS) {
	...
	$scope.addPmatter = function()
	{
		...
		ACS.addDmatter($scope.data).then(function (response) { if(response.data[0].Result=="Success"){alert(response.data[0].DisplayMessage); window.location = "index.html";}  });
	}
